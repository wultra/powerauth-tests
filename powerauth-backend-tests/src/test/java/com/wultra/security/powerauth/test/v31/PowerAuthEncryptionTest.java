...
class PowerAuthEncryptionTest {
    ...
    void signAndEncryptTest() throws Exception {
        ...
        for (StepItem item: stepLogger.getItems()) {
            if (item.name().equals("Decrypted Response")) {
                assertEquals("{\"data\":\"Server successfully decrypted data and verified signature, request data: hello, user ID: " + config.getUserV31() + "\"}", item.object());
                responseSuccessfullyDecrypted = true;
                break;
            }
        }
    ...
    void signAndEncryptStringDataTest() throws Exception {
        ...
        for (StepItem item: stepLogger.getItems()) {
            if (item.name().equals("Decrypted Response")) {
                assertEquals("\"Server successfully decrypted data and verified signature, request data: "+requestData+", user ID: " + config.getUserV31() + "\"", item.object());
                responseSuccessfullyDecrypted = true;
                break;
            }
        }
